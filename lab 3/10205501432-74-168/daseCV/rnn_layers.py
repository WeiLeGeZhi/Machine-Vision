...
def rnn_backward(dh, cache):
    ...
    for t in reversed(range(T)):
        step_cache = cache[t]
        dxt, dprev_h, dWxt, dWht, dbt = rnn_step_backward(dh[:, t, :] + dprev_h, step_cache)
        dx[:, t, :] = dxt
        dWx += dWxt
        dWh += dWht
        db += dbt
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
        xt = x[:, t, :]
        ht, ct, step_cache = lstm_step_forward(xt, ht, ct, Wx, Wh, b)
        h[:, t, :] = ht
        cache.append(step_cache)
...
def lstm_backward(dh, cache):
    ...
    N, T, H = dh.shape
    x, prev_h, prev_c, Wx, Wh, b, a, i, f, o, g, next_c = cache[0]
    D = x.shape[1]
    dx = np.zeros((N, T, D))
    dh0 = np.zeros((N, H))
    dWx = np.zeros((D, 4 * H))
    ...
    for t in reversed(range(T)):
        dh_t = dh[:, t, :] + dprev_h_t
        cache_t = cache[t]
        dx_t, dprev_h_t, dprev_c_t, dWx_t, dWh_t, db_t = lstm_step_backward(dh_t, dprev_c_t, cache_t)
