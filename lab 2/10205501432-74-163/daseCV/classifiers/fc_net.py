...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        dhid, grads['W2'], grads['b2'] = affine_backward(dsc, cache2)
        dX, grads['W1'], grads['b1'] = affine_relu_backward(dhid, cache1)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        layer_of_input_dim = input_dim
        for i, hd in enumerate(hidden_dims):
            self.params['W%d'%(i+1)] = weight_scale * np.random.randn(layer_of_input_dim, hd)
            self.params['b%d'%(i+1)] = weight_scale * np.zeros(hd)
            if self.normalization == 'batchnorm':
                self.params['gamma%d'%(i+1)] = np.ones(hd)
                self.params['beta%d'%(i+1)] = np.zeros(hd)
            layer_of_input_dim = hd
        self.params['W%d'%(self.num_layers)] = weight_scale * np.random.randn(layer_of_input_dim, num_classes)
    ...
    def loss(self, X, y=None):
        ...
        for layer in range(1, self.num_layers):
            if self.normalization == 'batchnorm':
                x_input, cache = affine_bn_relu_forward(x_input, self.params['W%d' % layer], self.params['b%d' % layer], self.params['gamma%d' % layer], self.params['beta%d' % layer], self.bn_params[layer - 1])
            else:
                x_input, cache = affine_relu_forward(x_input, self.params['W%d' % layer], self.params['b%d' % layer])
            hidden_layers.append(x_input)
            caches.append(cache)
        scores, final_cache = affine_forward(x_input, self.params['W%d' % self.num_layers], self.params['b%d' % self.num_layers])
        ...
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
         ...
         # Compute the data loss using softmax
        ...
        dx_input, grads['W%d' % self.num_layers], grads['b%d' % self.num_layers] = affine_backward(dscores, final_cache)
        grads['W%d' % self.num_layers] += self.reg * self.params['W%d' % self.num_layers]

        for layer in range(self.num_layers - 1, 0, -1):
            if self.normalization == 'batchnorm':
                dx_input, grads['W%d' % layer], grads['b%d' % layer], grads['gamma%d' % layer], grads['beta%d' % layer] = affine_bn_relu_backward(dx_input, caches.pop())
            else:
                dx_input, grads['W%d' % layer], grads['b%d' % layer] = affine_relu_backward(dx_input, caches.pop())
