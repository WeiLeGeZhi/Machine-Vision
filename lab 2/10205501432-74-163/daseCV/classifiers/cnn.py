...
class ThreeLayerConvNet(object):
    ...
    def __init__(self, input_dim=(3, 32, 32), num_filters=32, filter_size=7,
        ...
        pool_stride = 2
        pad = (filter_size-1) // 2
        H_conv = 1 + (H + 2 * pad - filter_size) // 1
        W_conv = 1 + (W + 2 * pad - filter_size) // 1
    ...
    def loss(self, X, y=None):
        ...
        dscores, dW2, db2 = affine_relu_backward(dscores, layer2_cache)
        dscores, dW1, db1 = conv_relu_pool_backward(dscores, conv_cache)
        dW3 += self.reg * W3
        dW2 += self.reg * W2
        dW1 += self.reg * W1
