...
class KNearestNeighbor(object):
    ...
    def compute_distances_two_loops(self, X):
        ...
        for i in range(num_test):
            for j in range(num_train):
                ...
                dists[i][j] = ((X[i]-self.X_train[j])@(X[i]-self.X_train[j]).T)**0.5
    ...
    def compute_distances_no_loops(self, X):
        ...
        dists=(np.sum(self.X_train**2,axis=1).reshape(1,num_train)+np.sum(X**2,axis=1).reshape(num_test,1)-2*X@self.X_train.T)**0.5
