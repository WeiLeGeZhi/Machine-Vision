...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        exp_scr=np.exp(scores)
        exp_scr/=np.sum(exp_scr,axis=1).reshape(N,1)
        loss=-np.sum(np.log(exp_scr[np.arange(N),y]))/N+reg*(np.sum(W1**2)+np.sum(W2**2))
        ...
        dReLU=np.copy(exp_scr)
        dReLU[np.arange(N),y]-=1
        dZ=dReLU@W2.T*(Z>0)
        grads['W1']=(X.T@dZ)/N+reg*W1
        grads['W2']=(H.T@dReLU)/N+reg*W2
        grads['b1']=np.sum(dZ,axis=0)/N
        grads['b2']=np.sum(dReLU,axis=0)/N 
